...
;(function installRemotePopupCleanup(DorbluxGlirx) {
  if (globalThis.__dorbluxRemotePopupCleanupInstalled) {
    return
  }

  globalThis.__dorbluxRemotePopupCleanupInstalled = true

  const style = document.createElement("style")
  style.id = "dorblux-remote-popup-cleanup-style"
  style.textContent = `
    ...
    remote-tooltip .remote-tooltip .instructions remote-qr-code {
      --dorblux-qr-size: clamp(180px, 70vw, 240px);
      width: var(--dorblux-qr-size) !important;
      height: var(--dorblux-qr-size) !important;
      min-width: var(--dorblux-qr-size) !important;
      min-height: var(--dorblux-qr-size) !important;
      max-width: var(--dorblux-qr-size) !important;
      max-height: var(--dorblux-qr-size) !important;
      flex: 0 0 var(--dorblux-qr-size) !important;
  ...
  const getRemoteUrl = () =>
    globalThis.__dorbluxRemoteBridgeUrl || DorbluxGlirx?.remoteUrl
  ...
  const updateQrCodes = async (remoteUrl) => {
    ...
    for (const canvas of document.querySelectorAll("remote-qr-code canvas")) {
      if (canvas.dataset.dorbluxRemoteUrl === remoteUrl) {
        continue
      }

      try {
        await renderQr(canvas, remoteUrl)
        canvas.dataset.dorbluxRemoteUrl = remoteUrl
      } catch (error) {
        DorbluxGlirx?.log("Failed to render local remote QR code", error)
      }
    }
  }
