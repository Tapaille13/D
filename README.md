<h4>An open-source interoperability tool designed to extend local client-side configurations and improve the UX of the Dorblux application.</h4>
...
## 🌐 Remote Web Panel
DorbluxGlirx includes a built-in **Remote Web Panel** allowing you to control app features directly from your phone.

### Quick Start:
1. Ensure both your PC and phone are on the **same Wi-Fi network**.
2. Hover over the **Connect** button in the top bar of DorbluxGlirx.
...
6. Extract the artifact zip and run `DorbluxGlirx.exe` to apply local client modifications.
...
You can inject your own JavaScript into Dorblux at patch time to tweak or fix things in the client UI. This reuses the same renderer injection the Remote Web Panel uses, so it requires the **Remote Web Panel** patch to be enabled.
...
Then patch as usual — your scripts are bundled into the client and run inside Dorblux's window.

**How it runs**

- Each script runs inside Dorblux's renderer (full DOM access, plus Node `require`).
- It is wrapped so a thrown error is logged and never crashes Dorblux.
- It may run **more than once** per launch (on load and again shortly after), so guard one‑time work behind a global flag.
- A small `DorbluxGlirx` helper is available: `DorbluxGlirx.log(...)`, `DorbluxGlirx.remoteUrl`, `DorbluxGlirx.apiVersion`.
...
if (!globalThis.__helloScriptInstalled) {
  ...
  DorbluxGlirx.log("Hello from my custom script!", DorbluxGlirx.remoteUrl);

  new MutationObserver(() => {
    const dialog = document.querySelector("ux-dialog:not([data-seen])");
    if (dialog) {
      dialog.setAttribute("data-seen", "1");
      DorbluxGlirx.log("A dialog opened.");
    }
  }).observe(document.documentElement, { childList: true, subtree: true });
}
```

> Scripts run with the same privileges as the Dorblux client. Only add scripts you trust and understand.
