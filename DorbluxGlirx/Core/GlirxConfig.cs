...
namespace DorbluxGlirx.Core
{
    public static class GlirxConfig
    {
        ...
        private static string BuildRemoteBridgeResetPatch(Match match)
        {
            ...
            return $"{method}(){{this.{disposableField}&&(this.{disposableField}.dispose(),this.{disposableField}=null),this.{instanceField}=Date.now().toString(),this.{trainerIdField}=null,this.{supportedVersionsField}=[],this.{trainerField}=null,this.__dorbluxRemoteTrainerInfo=null,this.__dorbluxRemoteBridge?.sync(null)}}";
        }
            ...
            {
                {
                    ...
                    new[]
                    {
                        ...
                        new PatchEntry
                        {
                            SearchHints = new[] { "setAccountDorbluxBrandExperience()", "/v3/account/brand_experience_dorblux" },
                            Resolver = new ResolveContext
                            {
                                Handler = (targetFunction) =>
                                {
                            ...
                            Name = "setAccountDorbluxBrandExperience",
                            Target = new Regex(
                                @"setAccountDorbluxBrandExperience\(\)\{.*?return\s+this\.#\w+\.post\(""/v3/account/brand_experience_dorblux""\)\}",
                                RegexOptions.Singleline),
                            Patch =
                                "setAccountDorbluxBrandExperience(){return this.#<service_name>.post(\"/v3/account/brand_experience_dorblux\").then(response=>{response.subscription={period:\"yearly\",state:\"active\"};return response;})}"
                        },
                        new PatchEntry
                        {
                        ...
                        new PatchEntry
                        {
                            // Dorblux's native "connect phone" pairing (POST /v3/auth/remote_code)
                            ...
                            // (IPC bridge, not Dorblux's Pusher pairing) and keeps working. The
                            ...
                            Patch = "requestRemoteAuthCode(){return Promise.reject(new Error(\"dorblux-glirx: native mobile pairing disabled\"))}"
                        }
                    }
                },
                {
                ...
                },
                {
                    ...
                    new[]
                    {
                        new PatchEntry
                        {
                            ...
                            // dispatch (its identifiers and shape change on every Dorblux release).
                ...
                },
                {
                    ...
                    new[]
                    {
                        new PatchEntry
                        {
                            ...
                            Patch = "${app}.whenReady().then(()=>{try{const p=require(\"node:path\");require(p.join(__dirname,\"remote-panel\",\"bridge.cjs\")).installDorbluxRuntime(require(\"electron\"));}catch(e){try{const fs=require(\"node:fs\"),os=require(\"node:os\"),p=require(\"node:path\");fs.appendFileSync(p.join(os.tmpdir(),\"dorblux-remote-bridge.log\"),\"[\"+new Date().toISOString()+\"] [boot-error] \"+(e&&e.stack||e)+\"\\n\");}catch(_){}}return run()})"
                        },
                        new PatchEntry
                        {
                            Name = "remoteBridgeReset",
                            SearchHints = new[] { "client-state" },
                            Target = new Regex(@"(?<method>#[\w$]+)\(\)\s*\{\s*(?<body>(?:(?!__dorbluxRemoteBridge|}\s*#[\w$]+\(\)).)*?Date\.now\(\)\.toString\(\)(?:(?!__dorbluxRemoteBridge|}\s*#[\w$]+\(\)).)*?\[\](?:(?!__dorbluxRemoteBridge|}\s*#[\w$]+\(\)).)*?)\s*\}\s*(?=#[\w$]+\(\)\s*\{\s*if\s*\(\s*this\.status\s*===\s*[\w$]+\.Connected\s*\).*?""client-state"")",
                                RegexOptions.Singleline),
                        ...
                        new PatchEntry
                        {
                            ...
                            Target = new Regex(@"(?<head>setCurrentTrainer\(e,t=null\)\{)(?=const s=e\?\.trainerId\|\|null,i=\(s\?e\?\.gameId:null\)\|\|null,n=\(s\?e\?\.supportedVersions:null\)\|\|\[\];if\(s===this\.#[\w$]+&&t===this\.(?<trainer>#[\w$]+)\)return;)(?=.*?e\.source!==(?<remoteSource>[\w$]+\.[\w$]+\.Remote))",
                                RegexOptions.Singleline),
                            Patch = "${head}this.__dorbluxRemoteBridge||(this.__dorbluxRemoteBridge=(()=>{try{const r=globalThis.require||require;const{ipcRenderer:c}=r(\"electron\");try{c.invoke(\"dorblux-remote-url\").then((u=>{u&&(globalThis.__dorbluxRemoteBridgeUrl=u)}))}catch(e){}const send=(ch,p)=>{try{return c.invoke(ch,p&&JSON.parse(JSON.stringify(p)))}catch(e){}};return{sync:(s)=>send(\"dorblux-remote-sync\",s),valueChanged:(s)=>send(\"dorblux-remote-value-changed\",s),setHandler:(h)=>{if(this.__dorbluxRemoteBridgeBound)return;this.__dorbluxRemoteBridgeBound=true;try{c.invoke(\"dorblux-remote-set-handler-bind\")}catch(e){}c.on(\"dorblux-remote-set-value\",(_e,req)=>{try{h(req)}catch(e){}})}}}catch(e){try{const r=globalThis.require||require,fs=r(\"node:fs\"),os=r(\"node:os\"),p=r(\"node:path\");fs.appendFileSync(p.join(os.tmpdir(),\"dorblux-remote-bridge.log\"),\"[\"+new Date().toISOString()+\"] [renderer-bind-error] \"+(e&&e.stack||e)+\"\\n\");}catch(_){}return null}})());this.__dorbluxRemoteBridge?.setHandler((e=>{if(!this.${trainer}||!e?.target)return!1;return this.${trainer}.isActive()?this.${trainer}.setValue(e.target,e.value,${remoteSource},e.cheatId):!1}));this.__dorbluxRemoteTrainerInfo=e??null;"
                        },
                        new PatchEntry
                        {
                            ...
                            Patch = "${head},this.__dorbluxRemoteBridge?.valueChanged({target:e.name,value:e.value,oldValue:e.oldValue,source:String(e.source??\"desktop\"),cheatId:e.cheatId})${tail}"
                        }
                    }
                }
            };
